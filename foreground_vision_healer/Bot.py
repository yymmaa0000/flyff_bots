...
class Bot:
    ...
    def __rebuff_timer(self):
        ...
        if ((currTime - self.mLastRebuffTime) >= (float(self.config["healing_interval_ms"]) * 60)):
            ...
            self.keyboard.press_key(VKEY["c"]) # cast action bar
            sleep(3)
            self.keyboard.hold_key(VKEY["5"], press_time=0.06) # equip knuckle back
            self.keyboard.hold_key(VKEY["6"], press_time=0.06) # equip shield
